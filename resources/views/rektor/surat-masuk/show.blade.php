    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex items-center text-[13px] text-[#64748B] mb-4 font-medium">
                <a href="{{ route('rektor.surat-masuk.index') }}" class="hover:text-[#4338CA] transition-colors">Persetujuan Surat</a>
            ...
            <div class="mb-6 flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div class="shrink-0">
                    <a href="{{ route('rektor.surat-masuk.index') }}" class="btn-secondary">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    {{-- Catatan Warek (Jika Ada) --}}
                    @if($suratMasuk->catatan_warek)
                        <div class="card-enterprise-lg p-6 sm:p-8">
                            <h3 class="text-base font-bold text-[#0F172A] mb-6 flex items-center gap-2 border-b border-[#F1F5F9] pb-4">
                                <i data-lucide="message-square" class="w-5 h-5 text-[#4338CA]"></i> Catatan Wakil Rektor
                            </h3>
                    ...
                    {{-- Form Disposisi Khusus Rektor Jika Disetujui --}}
                    @if($suratMasuk->status === 'menunggu_rektor' || $suratMasuk->status === 'selesai')
                        <div class="card-enterprise-lg p-6 sm:p-8 bg-[#F8FAFC] border-2 border-[#E2E8F0] shadow-sm">
                            <h3 class="text-base font-bold text-[#0F172A] mb-6 flex items-center gap-2 border-b border-[#E2E8F0] pb-4">
                                <i data-lucide="share-2" class="w-5 h-5 text-[#4338CA]"></i> Pengaturan Disposisi (Opsional)
                            </h3>

                            @if($suratMasuk->disposisis->count() > 0)
                                <div class="space-y-4 mb-6">
                                    <p class="text-[13px] font-semibold text-[#0F172A]">Disposisi yang sudah ditugaskan:</p>
                                    @foreach($suratMasuk->disposisis as $dis)
                                        <div class="bg-white border border-[#E2E8F0] rounded-xl p-3 flex items-start gap-3">
                                            <div class="shrink-0 pt-1">
                                                <i data-lucide="corner-down-right" class="w-4 h-4 text-[#94A3B8]"></i>
                                            </div>
                                            <div>
                                                <p class="text-[13px] font-bold text-[#0F172A] mb-1">{{ $dis->tujuanBagian->name }}</p>
                                                <p class="text-[12px] text-[#475569]">{{ $dis->instruksi }}</p>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <form action="{{ route('rektor.disposisi.store', $suratMasuk) }}" method="POST">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="block text-[13px] font-semibold text-[#0F172A] mb-2">Tujuan Bagian</label>
                                        <div class="relative">
                                            <select name="bagian_ids[]" multiple class="w-full border border-[#CBD5E1] rounded-xl px-4 py-3 text-[14px] text-[#0F172A] focus:outline-none focus:border-[#4338CA] focus:ring-4 focus:ring-[#4338CA]/10 appearance-none bg-white font-sans h-24">
                                    ...
                                    <div>
                                        <label class="block text-[13px] font-semibold text-[#0F172A] mb-2">Instruksi Disposisi</label>
                                        <input type="text" name="instruksi" required placeholder="Tulis instruksi ringkas..." class="input-enterprise w-full bg-white">
                                    </div>
                                </div>
                                <div class="flex justify-end">
                                    <button type="submit" class="btn-primary h-9 px-4 text-xs">
                                        <i data-lucide="send" class="w-3.5 h-3.5"></i> Tambah Disposisi
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    @if($suratMasuk->status === 'menunggu_rektor')
                        <div class="card-enterprise-lg p-6 sm:p-8 bg-white border-2 border-[#4338CA]/20 shadow-lg shadow-[#4338CA]/5">
                            <h3 class="text-base font-bold text-[#0F172A] mb-6 flex items-center gap-2 border-b border-[#F1F5F9] pb-4">
                                <i data-lucide="check-square" class="w-5 h-5 text-[#4338CA]"></i> Keputusan Rektor
                            </h3>
                            ...
                            <form method="POST">
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea name="catatan_rektor" rows="4" 
                                              ...
                                              placeholder="Tuliskan persetujuan akhir atau alasan penolakan..."></textarea>
                                </div>

                                <div class="flex flex-col sm:flex-row gap-3">
                                    <button type="submit" formaction="{{ route('rektor.surat-masuk.approve', $suratMasuk) }}" 
                                            class="flex-1 btn-primary bg-[#10B981] hover:bg-[#059669]">
                                        <i data-lucide="check-circle" class="w-4 h-4"></i> Setujui Surat
                                    </button>
                                    <button type="submit" formaction="{{ route('rektor.surat-masuk.kembalikan', $suratMasuk) }}" 
                                            class="flex-1 btn-secondary text-[#EF4444] hover:bg-[#FEE2E2] hover:border-[#FEE2E2]">
                                        <i data-lucide="x-circle" class="w-4 h-4"></i> Tolak & Kembalikan
                                    </button>
                                </div>
                            </form>
                        </div>
